...
class IssueDetailViewController: UIViewController {
    ...
    private func setupNavBarBlur() {
        ...
        navigationController?.navigationBar.addSubview(navBarBlurView)

        NSLayoutConstraint.activate([
            navBarBlurView.topAnchor.constraint(equalTo: (navigationController?.navigationBar.topAnchor)!, constant: -1 * navBarHeight),
            navBarBlurView.leadingAnchor.constraint(equalTo: (navigationController?.navigationBar.leadingAnchor)!),
            navBarBlurView.trailingAnchor.constraint(equalTo: (navigationController?.navigationBar.trailingAnchor)!),
            navBarBlurView.bottomAnchor.constraint(equalTo: (navigationController?.navigationBar.bottomAnchor)!),
        ])
    }
    ...
    private func setupContentBlur() {
        ...
        NSLayoutConstraint.activate([
            contentBlurView.topAnchor.constraint(equalTo: view.topAnchor),
            contentBlurView.leadingAnchor.constraint(equalTo: view.leadingAnchor),
            contentBlurView.trailingAnchor.constraint(equalTo: view.trailingAnchor),
            contentBlurView.bottomAnchor.constraint(equalTo: view.bottomAnchor),
        ])
    }
    ...
    @objc func editButtonTapped() {
        let previousData = PreviousData(title: issueDetailViewModel.title,
...
extension IssueDetailViewController: BottomSheetViewDelegate {
    ...
    func upButtonTapped() {
        ...
        if currentIndexPath.item == 0 {
            ...
            collectionView.scrollToItem(at: IndexPath(item: currentIndexPath.item - 1, section: 0), at: .top, animated: true)
        }
    }
    ...
    func downButtonTapped() {
        ...
        if currentIndexPath.item < issueDetailViewModel.comments.count - 1 {
            collectionView.scrollToItem(at: IndexPath(item: currentIndexPath.item + 1, section: 0), at: .top, animated: true)
        }
    }
