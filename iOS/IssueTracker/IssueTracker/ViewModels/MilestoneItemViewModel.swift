...
struct MilestoneItemViewModel: MilestoneCellConfigurable, MilestoneSubmitFormConfigurable {
    ...
    init(milestone: Milestone, from: DateFormatFrom) {
        ...
        switch from {
        case .fromServer:
            dueDate = milestone.dueDate.dateForServer
        case .fromSubmitView:
            dueDate = milestone.dueDate.dateForSubmitForm
        }
        issueOpened = Int(milestone.openedIssueNum) ?? 0
        issueClosed = Int(milestone.closedIssueNum) ?? 0
    }
